...
class ProductsService {
    ...
    func fetchCompanies(fromLocalJson: Bool = false) -> AnyPublisher<[Company], Error> {
        ...
        if fromLocalJson {
            ...
            guard let url = URL(string: "https://www.paytmmoney.com/mutual-funds/benefits") else {
    ...
    private func loadCompaniesFromFile(filename: String) -> AnyPublisher<[Company], Error> {
        Future { promise in
    ...
    private func loadProductDetailsFromFile(filename: String) -> AnyPublisher<ProductDetails, Error> {
        Future { promise in
            DispatchQueue.global(qos: .background).async {
                guard let url = Bundle.main.url(forResource: filename, withExtension: "json") else {
                    return promise(.failure(URLError(.fileDoesNotExist)))
